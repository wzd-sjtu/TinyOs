...
void pfree(uint32_t pg_phy_addr) {
   ...
   bitmap_set(&mem_pool->pool_bitmap, bit_idx, 0); // set the physical bitmap to zero
}

static void page_table_pte_remove(uint32_t vaddr) {
   uint32_t* pte = pte_ptr(vaddr); // vaddr's pte pointer
   *pte &= ~PG_P_1;
...
static struct arena* block2arena(struct mem_block* b) {
   return (struct arena*) ((uint32_t)b & 0xfffff000); // arena's size is always 1 page
}

void* sys_malloc(uint32_t size) {
   // malloc function's child function
   enum pool_flags PF;
   struct pool* mem_pool;
   uint32_t pool_size;
   ...
   if(cur_thread->pgdir == NULL) {
      ...
      mem_pool = &kernel_pool;
      descs = k_block_descs;  // num of 7 vector
   }
   else {
      PF = PF_USER;
   ...
   lock_acquire(&mem_pool->lock); // add the lock
   ...
   if(size > 1024) {
      ...
      if(a != NULL) {
         memset(a, 0, page_cnt*PG_SIZE); // continue address

         a->desc = NULL;
         a->cnt = page_cnt;
         a->large = 1;
         lock_release(&mem_pool->lock);
         return (void*)(a+1); // jump the arena struct a's size is an arena
      }
      else {
         lock_release(&mem_pool->lock);
         return NULL;
      ...
      if(list_empty(&descs[desc_idx].free_list)) {
         ...
         intr_set_status(old_status); // begin the intrrrupt
      }

      // begin to allocate the memory block
      b = elem2entry(struct mem_block, free_elem, list_pop(&descs[desc_idx].free_list));
      memset(b, 0, descs[desc_idx].block_size);

      a = block2arena(b);
      a->cnt--;
      lock_release(&mem_pool->lock); // release the lock
      return (void*)b;
   }
...
void sys_free(void* ptr) {
   if(ptr!=NULL) {
      ...
      if(a->desc == NULL && a->large == 1) {
         ...
         list_append(&a->desc->free_list, &b->free_elem); // add to the free_list
         if(++a->cnt == a->desc->blocks_per_arena) {
            uint32_t block_idx = 0;
            for(block_idx=0; block_idx < a->desc->blocks_per_arena; block_idx++) {
               struct mem_block* b = arena2block(a, block_idx);
               list_remove(&b->free_elem); // every free of block_mem
            }
            mfree_page(PF, a, 1); // it reveals memory control system
         }
      }
      lock_release(&mem_pool->lock); // release the lock for every one of us
   }
}
void mem_init() {
   ...
   block_desc_init(k_block_descs); // 初始化mem_block_desc;
