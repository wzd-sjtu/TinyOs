...
void* get_user_pages(uint32_t pg_cnt) {
   ...
   return vaddr; // user_pool envolves multiple compete usage
}
...
void* get_a_page(enum pool_flags pf, uint32_t vaddr) {
   ...
   if(cur->pgdir != NULL && pf == PF_USER) {
      ...
      // there is some wrong behavour happening
      ASSERT(1==2);
   }
   ...
   page_table_add((void*)vaddr, page_phyaddr); // connect the page together
